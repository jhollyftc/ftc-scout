...
export interface AlliancePick {
  teamNumber: number
  status: 'available' | 'taken' | 'declined'
  priority: number
}
...
export async function GET(_req: NextRequest, ctx: Ctx) {
  ...
  const { blobs } = await list({ prefix: `alliance/${season}/${eventCode}.json` })
...
export async function POST(req: NextRequest, ctx: Ctx) {
  ...
  const { season, eventCode } = await ctx.params
  const data: AlliancePick[] = await req.json()
  await put(
    `alliance/${season}/${eventCode}.json`,
    JSON.stringify(data),
